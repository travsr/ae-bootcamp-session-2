...
This document describes the testing guidelines for the TODO task-tracking app — a full-stack JavaScript application with a React frontend (`packages/frontend/`) and a Node.js/Express backend (`packages/backend/`). Refer to [Project Overview](./project-overview.md) for the full architecture and tech stack.
